...
class RinggroupModel extends \Model\Base {    
    ...
    public static function createList($data) {
		$validator = new Validator($data, array('ringgroup_id' => 'required', 'extentype' => 'required', 'dst_number' => 'number|extension', 'external_dst_number' => 'number'));
        if ($validator->fails()) {
            throw new \ValidationException($validator->all());
            return array('status' => 'ERROR', 'message' => 'Validation error');
        }
		...
		// Check if record already exists
		$res = \AD\Pbx\RigngroupAD::getList($data['ringgroup_id'], '', $data['extentype'], $data['dst_number']);
        if ($res['status'] == 'ERROR') {
			return $res;
		}
		if (isset($res['data'][0])) {
    ...
    public static function updateList($data) {
		...
		// Check if record exists
		$res = \AD\Pbx\RigngroupAD::getList($data['ringgroup_id']);
        if ($res['status'] == 'ERROR') {
			return $res;
		}
		if (!isset($res['data'][0])) {
